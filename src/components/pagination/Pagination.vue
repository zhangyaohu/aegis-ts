  ...
  <div ref="pagination">
		<span>{{`共\r\t${total}\r\t条`}}</span>
		 <el-dropdown @command="handleSizeChange">
			<span class="mh-pagination__size"
			     @mouseenter="dropdownIconClass = 'el-icon-caret-top'"
           @mouseleave="dropdownIconClass = 'el-icon-caret-bottom'">
				{{pageSize}}
				<i :class="[dropdownIconClass]"></i>
			</span>
      <el-dropdown-menu>
        <el-dropdown-item v-for="(size, index) in sizes"
				                  :key="index"
													:class="{'active-text': size === pageSize}"
													:command="size">{{size}}</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
  ...
  data() {
    return {
      ...
      quicknextIconClass: "el-icon-more",
			...
			dropdownIconClass: 'el-icon-caret-bottom'
    };
  },
  methods: {
    ...
    togglePager(event) {
      ...
      if (newPage !== currentPage) {
		...
		handleSizeChange(size) {
			//处理每页显示多少数据
			let _this = this;
			if(event === _this.pageSize) return;
